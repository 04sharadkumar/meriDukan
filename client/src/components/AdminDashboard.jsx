...
import { getNotificationsCount } from "../api/admin/getNotificationsCount";
...
const StatCard = ({ icon, label, value, link, trend, loading }) => (
  <Link
    to={link}
    className="block bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all duration-200 hover:border-blue-200"
  >
...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const [usersRes, productsRes, ordersRes, revenueRes, notificationsCount] =
        await Promise.all([
          axios.get("http://localhost:5000/api/admin/totalUser", { headers }),
          axios.get("http://localhost:5000/api/products/totalProduct", {
            headers,
          }),
          axios.get("http://localhost:5000/api/orders/totalOrder", { headers }),
          axios.get("http://localhost:5000/api/admin/totalRevenue", {
            headers,
          }),
          getNotificationsCount(),
        ]);

      // console.log(usersRes.data.totalUser, productsRes, ordersRes, revenueRes);
  ...
  const fetchRecentOrders = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5000/api/orders/getRecentOrders",
        { headers }
      );
