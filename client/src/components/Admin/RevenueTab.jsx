...
const RevenueTab = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get('http://localhost:5000/api/admin/getAllOrders', {
  ...
  const calculateMetrics = (orders, dateRange) => {
  ...
  const calculateRevenueTrend = (orders) => {
    ...
    orders.forEach(order => {
      const orderDate = new Date(order.createdAt);
      const monthIndex = (orderDate.getFullYear() - now.getFullYear()) * 12 + orderDate.getMonth() - now.getMonth() + 11;
          ...
          <div className="flex space-x-8">
            {['overview', 'products', 'customers', 'channels'].map((tab) => (
              <button
                ...
                className={`px-1 py-3 text-sm font-medium border-b-2 ${activeTab === tab ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              >
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {metrics.topProducts.map((product) => (
                        <tr key={product.name}>
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">${product.revenue.toFixed(2)}</td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{product.unitsSold}</td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">${(product.revenue / product.unitsSold).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {orders.slice(0, 5).map((order) => (
                        <tr key={order._id}>
                          <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">#{order._id.slice(-6)}</td>
